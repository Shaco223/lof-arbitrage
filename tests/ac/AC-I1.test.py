...
validate the list response. The dev-004 sample builder now emits the full PRD
1.2/1.3 field set, so this offline smoke validates the complete PRD 1.2/1.3
list contract (the same contract the real local API e2e suite enforces).
Pass criteria: p95 <= 800 ms, rows returned == watchlist-v2 size (currently
139 after PRD 1.5 expansion), and every list item matches the full PRD 1.2/1.3
list contract (types/enums/nullable).
Dependency: dev-004 local backend sample-output / uniCloud list function.
Current status: implemented as local full 1.2/1.3 contract smoke.
"""
...
def test_ac_i1_list_sample_output_matches_prd6_contract(project_root):
    ...
    assert len(response["data"]["items"]) == _watchlist_size(project_root)
